...
One-shot data migrations live beside the app (`migrate_personas.py`, `migrate_settings.py`, `migrate_api_models.py`, `migrate_drop_vendor.py`, `migrate_seed_story_mode.py`, `migrate_story_mode_v2.py`) and record completion in `_migrations`. New data migrations should be idempotent and called from `create_schema`. Use `builtin_id(kind, key)` to compute deterministic UUIDv5 ids when inserting/updating built-in rows from a migration, and gate writes on `is_builtin = 1` so user-duplicated copies stay untouched.

SQLite tuning lives in the same file: WAL journal_mode, `synchronous=NORMAL`, and `busy_timeout=15000` are set on every new connection. The 15 s wait is intentionally long enough for an autodrive task to finish responding to a cancel signal — `freeze_room` and `delete_room` both `asyncio.gather(..., return_exceptions=True)` over the cancelled tasks before mutating, so the lock has actually been released by the time the next write tries to grab it.
...
Autodrive is triggered from `after_message_appended` through `maybe_autodrive_after`: user-authored speech/question/answer/user_doc messages schedule one background `run_room_turn`, while AI-authored replies never recursively schedule another turn. Per-room autodrive locks prevent queue buildup; users can still explicitly call `/turn`. `_should_auto_discuss` chains additional turns inside the autodrive loop until either `consecutive_ai_turns >= max_consecutive_ai_turns`, the casual continuation probability fails, or the phase exits — **except for phases tagged `story`**, which skip the casual decay so a scene plays out continuously until the cap, the token budget, or a user freeze. `engine.is_autodrive_active(room_id)` exposes the lock state and `engine.schedule_autodrive(room_id)` lets the user manually kick the chain via `POST /rooms/{id}/autodrive/resume` without sending a message.

Multi-AI peer routing in `_build_messages` (`app/llm.py`) is the load-bearing fix that keeps personas from converging into a single narrator voice in multi-AI rooms: each model only sees its **own** past turns as `assistant`; every other persona's turn is rewritten to `user` with a `「Name」: ` prefix, and the system prompt gets a "you are only X" rule when peers exist. The engine resolves peer names from `PersonaInstance` rows and passes them as `peer_names` to `stream` / `complete_with_tools`. The casual silence escape valve (`<silent/>` sentinel) is also gated by phase tag: `casual_chat` keeps the lazy-silent default, but `story`-tagged phases get the opposite "stay in scene with even a beat of action" rule so the room doesn't collapse into a wall of silences.
...
`complete_tool` has a three-tier fallback so quirky providers don't blow up structured calls: (1) preferred forced `tool_choice={"type":"function",...}`, (2) on a 400 mentioning `tool_choice` (e.g. `deepseek-reasoner`) retry with `tool_choice="auto"` and a stronger nudge, (3) if the model still doesn't emit a tool call, drop tools entirely and ask for raw JSON conforming to the schema. `_parse_tool_arguments` strips ```` ```json ```` fences, slices to the first `{`/last `}` for prose-prefixed responses, and `_unstring_nested` recursively re-hydrates JSON-string sub-fields — MiMo and some OpenRouter relays double-encode nested objects/arrays inside tool arguments, this catches that without breaking legitimate text fields.

The persona-draft assistant (`POST /assistants/template-draft` with `kind=persona`) uses a tightly constrained `PersonaDraftEnvelope` (`schemas.py`) so the LLM cannot return free-form text — name length, prompt length, color hex regex, icon enum, temperature/talkativeness ranges all enforced. Errors are surfaced as 502 with the real reason (no silent fallback) so the UI can display them.
...
`app/seed.py` defines all built-in personas, phase templates, debate formats, and recipes. They are inserted on first startup keyed by deterministic UUIDv5 ids (`builtin_id(kind, key)`), and `seed_builtins` only seeds a template table when that table is empty — there is no upsert. To change a built-in payload after the dev DB is seeded you have to either delete the row/table, change the seed key, or write a one-shot migration (the `migrate_seed_story_mode*` pair are the working examples). Built-ins reference each other by these deterministic ids (e.g. format → phase template), so renaming a key is a breaking change. The persona taxonomy (12 personas across 7 categories) is documented in `docs/personas.md`. Each built-in persona carries `color` (hex) and `icon` (lucide name from the curated set in `frontend/src/components/PersonaIcon.tsx`); the frontend's `PersonaIcon` component renders them and the curated icon list MUST stay in sync with `schemas.PERSONA_ICON_NAMES`.

Built-in phases now include `story_mode` (casual ordering, no decay, only `user_manual` exit) and the `story_format` that wraps it; rooms created with this format autodrive continuously until the consecutive cap or freeze.
...
Server-pushed updates flow over a single SSE stream at `/rooms/{id}/events` (see `app/event_bus.py` and `frontend/src/hooks.ts::useRoomEvents`); on most event kinds the hook just invalidates the `["room", roomId]` React Query key — only `message.streaming` mutates Zustand directly to drive the live-typing UI. Invalidations are coalesced through a 250 ms debounce so an autodrive burst of `message.appended` events doesn't flood the backend with `/state` polls. Adding a new event type means updating both the publisher in `engine.py` / `main.py` and the switch in `useRoomEvents`.

`/rooms` (list endpoint) returns `RoomSummaryOut` (extends `RoomOut`) with bulk-aggregated `members` (PersonaIcon previews), `member_count`, `message_count`, and `last_activity_at` — all computed in 4 SQL calls, no N+1 over rooms, so the sidebar can render rich cards without per-room `/state` lookups. `RoomRuntimeOut` carries `autodrive_active` (lock state) and `current_speakers` (in-flight persona ids — populated even before the first chunk, which `in_flight_partial` cannot represent).
...
Room UI is composed in `frontend/src/pages/room/RoomShell.tsx` (three-column layout: `RoomListSidebar` / `MessageList` + `Composer` / `RightPanel`) and a set of right-rail panels under `frontend/src/pages/room/panels/` (Scribe, Facilitator, Decisions, PhasePlan, Subroom, Upload, Limit, Tool). Above the message list, `SpeakerStateBar` renders the runtime state machine (frozen / speaking / scheduling / idle) tinted with the current speaker's persona color, and provides the "让 AI 继续" button that calls `/autodrive/resume`. `RoomListSidebar` cards (not bars) show stacked `PersonaIcon` member previews, message count, and last-activity timestamp — driven by `RoomSummaryOut` from `/rooms`. `pages/RoomPage.tsx` is a thin wrapper — extend the panels rather than the page. The shared `frontend/src/components/` directory holds reusable UI primitives (`AppRail`, `MarkdownBlock`, `StatusPill`, `MentionChip`, `PhaseStepper`, `SectionCard`, `ConfirmDialog`, `Toaster`, `PersonaIcon`).
...
`AGENTS.md` is a near-verbatim copy of this file aimed at Codex. When you change architecture-level guidance here, mirror it there (or vice versa) so the two assistants don't drift.
