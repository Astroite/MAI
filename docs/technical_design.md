...
1. **人是速度瓶颈** —— 单房间内任意时刻至多一个 in-flight LLM call，房间内无并发。
...
| 类别 | 特性 | v1 实现 |
|------|------|---------|
| 持久业务数据 | 长期存储，跨会话 | Postgres 业务表 |
| 房间运行时状态 | 频繁读写 | 进程内存 + Postgres 双写 |
...
**B. 房间运行时状态：进程内存 + Postgres 双写**——schema 上把运行时字段放在独立的 `room_runtime_state` 表，将来真有压力时可整表迁移到 Redis。
...
**E. In-flight 状态只放内存**——进程崩溃则该 in-flight call 视为失败，前端用户重试即可。不做断点续传。
...
messages(
  ...
  user_revealed_at timestamptz null,  -- 伪装揭示时间(仅用户视图用)
...
| `mention_driven` | 等用户 @ 指定下一发言者 |
| `question_paired` | 上一句若是 question，下一句必须是被 @ 的回答方 |
| `parallel` | 所有 allowed 同时发言(每条独立 LLM call，结果合并) |
...
async def freeze_room(room_id):
    ...
    # 1. 取消当前 in-flight call
    if runtime.in_flight_call:
        await runtime.in_flight_call.cancel()  # stream.aclose() + task.cancel()
        # finally 块里处理 partial 保存
    
    # 2. 取消所有 background tasks
...
| @ 提及下一发言者 | ✅ 缓冲 | 写入 pending_user_action，stream 完成后处理 |
...
书记官、副手都不知情伪装。它们看到的是普通 AI 消息(role / 内容 / 时序无差异)。AI 可能反向推断身份，作为产品特性接受，不做对抗。
...
async def run_room_turn(room_id: str, trigger: UserAction):
    ...
    # 1. 处理 streaming 期间的缓冲操作
    if trigger.type == "user_action_during_stream":
        runtime.pending_user_action = trigger
        return
    
    # 2. 调度
    speaker = await pick_next_speaker(
        phase_instance=room.current_phase,
        room=room,
        last_action=trigger,
    )
    
    if speaker == WAIT_FOR_USER:
        return
    if speaker == PHASE_DONE:
        await emit_phase_exit_event(room)
        return
    
    # 3. 构造 context (应用 visibility 过滤,得到 AI 视图)
    context = build_ai_context(speaker, room.messages, room.scribe_state)
    
    # 4. 调用 LLM (streaming)
    in_flight = LLMCall(speaker, context, max_tokens=room.config.max_message_tokens)
    runtime.in_flight_call = in_flight
    ...
    try:
        ...
        # 6. 异步触发书记官 / 副手(非阻塞)
        asyncio.create_task(maybe_trigger_scribe(room_id, msg))
        asyncio.create_task(maybe_trigger_facilitator(room_id, msg))
        
        # 7. 检查 phase 是否结束
        await check_phase_exit(room)
    
    finally:
        runtime.in_flight_call = None
        
        # 8. 处理 streaming 期间缓冲的用户操作
        if runtime.pending_user_action:
            pending = runtime.pending_user_action
            runtime.pending_user_action = None
            await run_room_turn(room_id, pending)
    
    # 不主动调度下一轮 —— 等用户下一次 trigger
```
...
SSE manager hook
  ↓
按事件类型分发:
  - message.streaming → push chunk 到 Zustand 的 message_chunks
